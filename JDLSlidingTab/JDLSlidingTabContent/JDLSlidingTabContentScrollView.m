//
//  JDLSlidingTabContentScrollView.m
//  JDLSlidingTabDemo
//
...
#import "JDLSlidingTabContentScrollView.h"
#import "UIView+JDLExtension.h"

@interface JDLSlidingTabContentScrollView () <UIScrollViewDelegate>
// 外界父控制器
...
@implementation JDLSlidingTabContentScrollView

- (instancetype)initWithFrame:(CGRect)frame parentVC:(UIViewController *)parentVC childVCs:(NSArray *)childVCs {
    if (self = [super initWithFrame:frame]) {
        if (parentVC == nil) {
            @throw [NSException exceptionWithName:@"JDLSlidingTabContentScrollView" reason:@"JDLSlidingTabContentScrollView 所在控制器必须设置" userInfo:nil];
        }
        self.parentViewController = parentVC;
        if (childVCs == nil) {
            @throw [NSException exceptionWithName:@"JDLSlidingTabContentScrollView" reason:@"JDLSlidingTabContentScrollView 子控制器必须设置" userInfo:nil];
        }
...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        ...
        CGFloat contentWidth = self.childViewControllers.count * _scrollView.jdl_width;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    childVC.view.frame = CGRectMake(offsetX, 0, self.jdl_width, self.jdl_height);
}
...
- (void)setPageContentScrollViewCurrentIndex:(NSInteger)currentIndex {
    self.isClickBtn = YES;
    CGFloat offsetX = currentIndex * self.jdl_width;
