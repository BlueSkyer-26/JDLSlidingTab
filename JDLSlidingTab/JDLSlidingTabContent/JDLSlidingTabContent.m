...
#import "JDLSlidingTabContent.h"
#import "UIView+JDLExtension.h"

@interface JDLSlidingTabContent () <UICollectionViewDataSource, UICollectionViewDelegate>
/// 外界父控制器
...
@implementation JDLSlidingTabContent

- (instancetype)initWithFrame:(CGRect)frame parentVC:(UIViewController *)parentVC childVCs:(NSArray *)childVCs {
    if (self = [super initWithFrame:frame]) {
        if (parentVC == nil) {
            @throw [NSException exceptionWithName:@"JDLSlidingTabContent" reason:@"JDLSlidingTabContent 所在控制器必须设置" userInfo:nil];
        }
        self.parentViewController = parentVC;
        if (childVCs == nil) {
            @throw [NSException exceptionWithName:@"JDLSlidingTabContent" reason:@"JDLSlidingTabContent 子控制器必须设置" userInfo:nil];
        }
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        CGFloat collectionViewW = self.jdl_width;
        CGFloat collectionViewH = self.jdl_height;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    // 3、pageContentViewDelegare; 将 progress／sourceIndex／targetIndex 传递给 jdl_PageTitleView
...
#pragma mark - - - 给外界提供的方法，获取 jdl_PageTitleView 选中按钮的下标
- (void)setPageContentViewCurrentIndex:(NSInteger)currentIndex {
    self.isClickBtn = YES;
    CGFloat offsetX = currentIndex * self.collectionView.jdl_width;
    // 1、处理内容偏移
